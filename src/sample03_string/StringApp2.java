...
public class StringApp2 {
	...
	public static void main(String[] args) {
		...
		//String toLowerCase() : 문자열에 대해서 소문자로 반롼된 새 문자열을 반환한다.
		...
		//str1이 소문자로 변환된 것이 아닌 새로운 객체가 생성된 것이다.
		System.out.println(str1); //str1은 변하지 않았다.
		...
		//boolean isEmpty() : 문자열이 빈 문자열이면 true를 반환환다. 길이가 0인 문자열일 때 true를 반환한다.
		//boolean isBlank() : 문자열이 빈 문자열이면 true를 반환한다. 길이가 0인 문자열과 공백, 탭만 포함하고 있을 때 true를 반환한다.
		
		System.out.println("".isEmpty()); //길이가 0인 문자열, 빈 문자열이 있을 때 true // true
		System.out.println("  ".isEmpty()); // false, 공백문자가 여러개 포함되어 있기 때문에 빈 문자열이 아니다.
		System.out.println("".isBlank()); //공백 또한 비어있다고 판단한다 //true
		...
		String text3 = str2.substring(12); // 0부터 시작되며 공백 또한 카운팅한다.
		String text4 = str2.substring(2,9); //두번째 변수를 입력하면 메소드 중복정의가 되어있다는 것을 알 수 있다.
		...
		//int indexOf(Sting text) : 문자열에서 지정된 문자열이 등장하는 인덱스를 반환한다.
		//int indexOf(Sting text, int fromIndex) : 지정된 위치에서부터 검색했을 때 지정된 문자열이 등장하는 인덱스를 반환한다.
		...
		//int lastIndexOf(Sting text) : 
		//int lastIndexOf(Sting text, int fromIndex) : 
		String str3 = "Returns the char value at the specified index";
		int index1 = str3.indexOf('t');
		int index2 = str3.indexOf('t',20); //20번째 이후부터 t를 찾아라
		int index3 = str3.indexOf("the");
		int index4 = str3.indexOf("the", 20); //20번째 이후부터 the를 찾아라
		...
		int index6 = str3.lastIndexOf("i"); // 끝에서부터 찾는다.
...
//		char ch3 =  str4.charAt(20); // 문자열의 인덱스범위를 벗어난 값은 StringIndexOutofBoucdException 오류가 발생한다.
		System.out.println("5번째 문자 - > " + ch1 );
		System.out.println("10번째 문자 - > " + ch2 );
//		System.out.println("20번째 문자 - > " + ch3 ); // 20번째의 값은 없다.
		
		//String[] split(String index) : 문자열을 구분자(지정된 표현식)로 잘라서 배열에 담아 반환한다.
		...
		String str7 = "https://www.daum.net";
		...
		//String replace(CharSequence target,CharSequence replacement) : 문자열에서 target에 해당하는 내용을 replacement의 내용으로 변경한 새 문자열을 반환한다.
		String str11 = "I have macbook air and macbook pro";
		String str12 = str11.replace("macbook", "galaxy");// 원본을 참고해서 새로운 문자열을 만든다.
		System.out.println("원본 문자열 - > " + str11);
		System.out.println("새 문자열 - > " + str12);
		
		//String replaceAll(String regexp, String replacement)
		//문자열에서 정규표현식 패턴에 해당하는 내용을 지정된 문자열로 반환한다.
		String str13 = "홍길동의 집 전화번호는 02-1111-1111이고, 핸드폰 번호는 010-222-2222번이다";
		String str14 = str13.replaceAll("-\\d{3,4}-","-****-"); // \\d(숫자), {3,4}(3개 또는 4개) 정규표현식 검색
